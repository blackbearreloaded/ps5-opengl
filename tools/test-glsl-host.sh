...
for fault in id tag uv draw; do
    case "$fault" in
        id) change='s/float(gl_PrimitiveID)/float(0)/';;
        tag) change='s/glUniform1i(tag, 7)/glUniform1i(tag, 6)/';;
        uv) change='s/uv=a_position/uv=vec2(0)/';;
        draw) change='s/cases\[i\].count);/0);/';;
    esac
    sed "$change" "$source" |
...
echo 'GLSL host PASS: math/texture/PrimitiveID/program restoration; four faults rejected'
