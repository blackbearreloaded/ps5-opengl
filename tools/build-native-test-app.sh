...
[[ $gate_object =~ ^egl_public_core33_(imgui(_tv)?|nanovg|sokol)\.o$ ]] || grep -qxF "${gate_object}:" < <(
    grep -oE '^egl_public_[A-Za-z0-9_]+\.o:' "$root/tests/ps5/Makefile"
) || {
...
if [[ $gate_object =~ ^egl_public_core33_(imgui(_tv)?|nanovg|sokol)\.o$ ]]; then
    renderer=${gate_object#egl_public_core33_}
    renderer=${renderer%.o}
    renderer=${renderer%_tv}
    prefix="$root/build/sdk/ps5-opengl-core33"
