...
[[ $gate_object =~ ^egl_public_core33_(imgui(_tv)?|nanovg|sokol(_cube)?)\.o$ ]] || grep -qxF "${gate_object}:" < <(
    grep -oE '^egl_public_[A-Za-z0-9_]+\.o:' "$root/tests/ps5/Makefile"
) || {
...
if [[ $gate_object =~ ^egl_public_core33_(imgui(_tv)?|nanovg|sokol(_cube)?)\.o$ ]]; then
