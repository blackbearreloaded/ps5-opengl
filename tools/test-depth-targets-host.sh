...
targets="$root/tests/ps5/egl_public_core33_depth_targets.c"
targets_query_mismatch=0

for test in array mip targets; do
    "${CC:-cc}" "${flags[@]}" "${!test}" -l:libEGL.so.1 -l:libGL.so.1 -lm -o "$out/$test"
    if ! "$out/$test" > "$out/$test.log" 2>&1; then
        cat "$out/$test.log"
        # Only this exact system-Mesa query discrepancy may continue to the
        # fault lane. The source oracle remains failed and strict at layer 2.
        [[ "$test" == targets ]] || exit 1
        grep -qxF '[host-egl-core33-depth-targets] 1d-array selected=2 query=0 expected=2 routing=1 attachment=1 error=0x0 query_discrepancy=1' "$out/targets.log"
        grep -qxF '[host-egl-core33-depth-targets] matching=3 depths=0.500000/0.500000/0.500000/0.500000 draw=0/4 result=1' "$out/targets.log"
        grep -qxF '[host-egl-core33-depth-targets] cleanup=1 result=1' "$out/targets.log"
        targets_query_mismatch=1
    fi
...
if (( ! targets_query_mismatch )); then
    grep -qxF '[host-egl-core33-depth-targets] 1d-array selected=2 query=2 expected=2 routing=1 attachment=1 error=0x0 query_discrepancy=0' "$out/targets.log"
    grep -qxF '[host-egl-core33-depth-targets] matching=4 depths=0.500000/0.500000/0.500000/0.500000 draw=0/4 result=0' "$out/targets.log"
    grep -qxF '[host-egl-core33-depth-targets] cleanup=1 result=0' "$out/targets.log"
fi
...
for fault in raw-scalar array-data missing-draw valid-3d targets-missing-draw; do
    case "$fault" in
        ...
        targets-missing-draw) source="$targets"; change='s/glDrawArrays(GL_TRIANGLES, 0, 3)/glDrawArrays(GL_TRIANGLES, 0, 0)/'; tag=targets;;
    esac
    sed "$change" "$source" |
...
grep -qxF '[host-egl-core33-depth-targets] matching=0 depths=0.750000/0.750000/0.750000/0.750000 draw=0/4 result=1' "$out/targets-missing-draw.log"
if (( targets_query_mismatch )); then
    echo 'Depth targets host KNOWN QUERY MISMATCH: system Mesa returned 0 for selected 1D-array layer 2; routing/pixels/other attachments passed; five faults rejected; strict oracle remains FAIL (exit 1)'
    exit 1
fi
echo 'Depth targets host PASS: array mip/raw/shadow pixels, five depth mip targets, four depth attachment targets, invalid 3D rejected; five faults rejected'
