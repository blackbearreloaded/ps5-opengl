...
enum { PIPE_TEXTURE_2D, PIPE_TEXTURE_2D_ARRAY, PIPE_TEXTURE_3D };
...
struct ps5_resource { struct pipe_resource base; uint8_t *data,*stencil_data; size_t allocation_size,stencil_allocation_size; };
...
'''
for name in ('ps5_tiled_depth_layer_xor', 'ps5_tiled_surface_size',
             ...
             'ps5_depth_blit_layer', 'ps5_resolve_depth_stencil_msaa4',
...
static void run(unsigned mode,unsigned mask,unsigned source_layer,unsigned dest_layer,unsigned variant,unsigned packed) {
    ...
    for (unsigned layer=0;layer<LAYERS;++layer) for (unsigned sample=0;sample<src_samples;++sample)
        for (unsigned y=0;y<W;++y) for (unsigned x=0;x<W;++x) {
            ...
            memcpy(spans[0].p+offset(src_samples,0,x,y,sample,layer),&depth,4);
            spans[1].p[offset(src_samples,1,x,y,sample,layer)]=(uint8_t)(layer*29+sample*13+y+x);
        }
    ...
    if (variant==6) src.allocation_size=source_layer*65536u+65535u;
    if (variant==7) src.stencil_allocation_size=source_layer*65536u+65535u;
    if (variant==6 && !(mask & PIPE_MASK_Z)) valid=true;
    if (variant==7 && !(mask & PIPE_MASK_S)) valid=true;
    if (variant==8) { info.render_condition_enable=true; ctx.condition=false; }
    if (variant==9) info.src.level=1;
    if (variant==10) dst.base.last_level=1;
    ...
    if (valid) for (unsigned y=0;y<(unsigned)info.dst.box.height;++y) for (unsigned x=0;x<(unsigned)info.dst.box.width;++x) {
        ...
        for (unsigned sample=0;sample<dst_samples;++sample) {
            if (mask & PIPE_MASK_Z) memcpy(expected[2]+64+offset(dst_samples,0,dx,dy,sample,dest_layer),
                spans[0].p+offset(src_samples,0,sx,sy,0,source_layer),4);
            if (mask & PIPE_MASK_S) expected[3][64+offset(dst_samples,1,dx,dy,sample,dest_layer)]=
                spans[1].p[offset(src_samples,1,sx,sy,0,source_layer)];
        }
    }
    ...
    for (unsigned i=0;i<4;++i) {
        if (memcmp(raw[i],expected[i],PLANE+128)) {
            fprintf(stderr,"mode=%u mask=%u layers=%u/%u variant=%u packed=%u plane=%u\n",
                mode,mask,source_layer,dest_layer,variant,packed,i); abort();
        }
...
int main(void) {
    ...
    for (unsigned variant=0;variant<12;++variant) run(mode,mask,source,dest,variant,packed);
}
'''
...
print('PASS: actual depth blit/resolve/replicate branches, layer 0/2/3, D32/D32S8, '
      'masks, scissor, flip/scale, guards and invalid inputs; AMD-table oracle; '
      'old layer rejection and missing slice XOR rejected')
