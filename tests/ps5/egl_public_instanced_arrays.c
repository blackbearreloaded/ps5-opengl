...
compile_shader(GLenum type, const char *source, GLuint *result)
{
   ...
   if (!compiled) {
      ...
      printf("[ps5-egl-instanced-arrays] shader type=0x%x compile=0 log=%.*s\n",
             type, length, log);
            ...
            uint32_t expected_hash, const char *name)
{
   ...
   printf("[ps5-egl-instanced-arrays] %s matching=%u unexpected=%u hash=%08x error=0x%x\n",
...
main(void)
{
   static const char *vertex_source =
      ...
      "void main() {\n"
      "  gl_Position = vec4(a_position + vec2(a_instance.x, 0.0), 0.0, 1.0);\n"
      "  v_color = a_instance.yzw;\n"
      "}\n";
   ...
   if (!gl_version || !glsl_version || !extensions)
      goto cleanup;
   extension_present = has_extension(extensions, "GL_ARB_instanced_arrays");
   printf("[ps5-egl-instanced-arrays] egl=%d.%d surface=%dx%d context=%d gl=%s glsl=%s extension=%d\n",
   ...
   glBindAttribLocation(program, 1, "a_instance");
...
cleanup:
   ...
   printf("[ps5-egl-instanced-arrays] cleanup gl=0x%x egl=0x%x ok=%u result=%d\n",
