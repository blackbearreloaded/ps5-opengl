...
"""Pin the narrow native RGBA8 layout policy; other sampled images stay linear."""
...
enum { PIPE_FORMAT_R8G8B8A8_UNORM=1, OTHER_COLOR,
...
static bool ps5_sampled_texture_format(unsigned f) { return f >= 1 && f <= 4; }
''' + helper + r'''
int main(void) {
    assert(!ps5_linear_sampled_layout(NULL));
    const unsigned sizes[] = {1, 2, 63, 128, 1920, 3840, 8192, 8193};
    for (unsigned t=1; t<=5; ++t) for (unsigned f=1; f<=4; ++f)
    ...
    for (unsigned x=0; x<8; ++x) for (unsigned y=0; y<8; ++y) {
        ...
        bool expected = samples == 1 && (!(bind & PIPE_BIND_DEPTH_STENCIL) ||
        ...
        if (t == PIPE_TEXTURE_2D && f == PIPE_FORMAT_R8G8B8A8_UNORM &&
...
with tempfile.TemporaryDirectory() as temp:
    ...
    for render, dynamic in ((0, 0), (1, 0), (0, 1), (1, 1)):
...
print('PASS: native RGBA8 layout bounds, targets, binds, mips, samples and disabled feature paths')
