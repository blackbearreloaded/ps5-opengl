...
compile_shader(GLenum type, const char *source, GLuint *result)
{
   ...
   if (!compiled) {
      ...
      printf("[ps5-egl-snorm16-probe] shader type=0x%x compile=0 log=%.*s\n",
             type, length, log);
...
main(void)
{
   ...
   static const int16_t vertices[6] = {
      -16384, -16384,
       16384, -16384,
           0,  16384,
   };
   ...
   GLint attrib_normalized = GL_FALSE;
   ...
   if (!gl_version || !glsl_version)
      goto cleanup;
   printf("[ps5-egl-snorm16-probe] egl=%d.%d surface=%dx%d context=%d gl=%s glsl=%s\n",
   ...
   glVertexAttribPointer(0, 2, GL_SHORT, GL_TRUE, -1, NULL);
   invalid_error = glGetError();
   if (invalid_error != GL_INVALID_VALUE)
      goto cleanup;

   glVertexAttribPointer(0, 2, GL_SHORT, GL_TRUE,
                         2 * sizeof(int16_t), NULL);
   ...
   printf("[ps5-egl-snorm16-probe] invalid=0x%x type=0x%x size=%d stride=%d normalized=%d setup=0x%x\n",
          invalid_error, attrib_type, attrib_size, attrib_stride,
          attrib_normalized, setup_error);
   if (attrib_type != GL_SHORT || attrib_size != 2 ||
       attrib_stride != 2 * (GLint)sizeof(int16_t) || !attrib_normalized ||
   ...
   printf("[ps5-egl-snorm16-probe] public-gl matching=%u unexpected=%u hash=%08x error=0x%x\n",
...
cleanup:
   ...
   printf("[ps5-egl-snorm16-probe] cleanup gl=0x%x egl=0x%x ok=%u result=%d\n",
