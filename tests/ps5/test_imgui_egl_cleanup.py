...
unsigned calls;
unsigned snapshots;
void snapshot(const char *phase, unsigned sample) {
...
int probe(int injected, int result, bool observer) {
    failure = injected;
    calls = 0;
    snapshots = 0;
    ...
    (void)cleanup;
''' + cleanup + r'''
    return calls == 15 && snapshots == observer ? result : 2;
}
int main() {
    for (unsigned observer = 0; observer <= 1; ++observer)
    for (int failure = 0; failure <= 5; ++failure)
        for (int render_failure = 0; render_failure <= 1; ++render_failure)
            if (probe(failure, render_failure, observer) != (failure != 0 || render_failure)) {
