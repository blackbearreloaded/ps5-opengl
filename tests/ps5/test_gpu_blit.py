...
enum { PIPE_TEXTURE_2D=2, PIPE_FORMAT_R8G8B8A8_UNORM=1,
       PIPE_MASK_RGBA=15, PIPE_TEX_FILTER_NEAREST=0,
...
#define REJECT(field,value) do { struct pipe_blit_info bad=b; bad.field=value; \
    ...
    REJECT(src.resource,NULL); REJECT(dst.resource,NULL); REJECT(mask,7); REJECT(filter,1);
    ...
    REJECT(dst.box.width,513); REJECT(src.box.height,0); REJECT(dst.resource,&r.base);
