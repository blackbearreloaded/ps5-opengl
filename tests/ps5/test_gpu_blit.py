...
code += r'''
static struct pipe_resource *drained[16];
static void ps5_draw_batch_drain_buffer(struct pipe_resource *r) {
    assert(drains < 16); drained[drains++] = r;
}
...
#define QUERY_CONTEXT(field) do { c.field=1; assert(ps5_blit_gpu_color(&c,&b)); \
    ...
    for (unsigned format=5;format<=10;++format) {
        ...
        t.base.nr_samples=0;
        t.data++; assert(!ps5_linear_color_pitch(&s));
    }
