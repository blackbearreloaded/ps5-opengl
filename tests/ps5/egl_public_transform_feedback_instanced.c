...
compile_shader(GLenum type, const char *source, GLuint *result)
{
   ...
   if (!ok) {
      ...
      printf("[ps5-egl-xfb-instanced] compile type=0x%x log=%.*s\n",
             type, length, log);
...
main(void)
{
   static const char *vertex_source =
      ...
      "out vec3 captured;\n"
      "void main() {\n"
      "  captured = vec3(a_position, float(gl_InstanceID));\n"
   ...
   static const float expected[27] = {
   ...
   error = glGetError();
   if (captured) {
      values = (const float *)(captured + CAPTURE_OFFSET);
      prefix_ok = !memcmp(captured, initial, CAPTURE_OFFSET);
   ...
   if (prefix_ok && values_ok && suffix_ok && query_result[0] == 3 &&
       query_result[1] == 3 && draw_status == 0 && draw_calls == 3 &&
       error == GL_NO_ERROR)
      passed = 1;

   printf("[ps5-egl-xfb-instanced] core=%d.%d offset=%d "
          "canary=%d/%d values_ok=%d queries=%u/%u "
          "first=%g,%g,%g last=%g,%g,%g "
          "draw_status=%d submissions=%u error=0x%x\n",
          context_major, context_minor, CAPTURE_OFFSET, prefix_ok, suffix_ok,
          values_ok, query_result[0], query_result[1],
          values ? values[0] : 0.0, values ? values[1] : 0.0,
          values ? values[2] : 0.0, values ? values[24] : 0.0,
          values ? values[25] : 0.0, values ? values[26] : 0.0,
          draw_status, draw_calls, error);
   if (captured && glUnmapBuffer(GL_TRANSFORM_FEEDBACK_BUFFER) != GL_TRUE)
      passed = 0;
   captured = NULL;
...
cleanup:
   ...
   printf("[ps5-egl-xfb-instanced] result=%d\n", passed ? 0 : 1);
   return passed ? 0 : 1;
}
